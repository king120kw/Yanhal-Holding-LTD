...
export default function LeadershipCard({ name, role, coverUrl, characterUrl, index, onClick }: LeadershipCardProps) {
  ...
  return (
    <AnimatedBlock delay={0.2} index={index}>
      <motion.div 
        className="leadership-card group relative w-full max-w-[460px] mx-auto aspect-[3/4] sm:aspect-[4/5] cursor-pointer"
        initial="initial"
      ...
      >
        <motion.div 
          ...
          className="leadership-wrapper absolute inset-0 w-full h-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-charcoal"
        ...
        >
          ...
          >
            <h3 className="text-base sm:text-lg lg:text-xl font-display font-bold text-white uppercase tracking-wide mb-1 drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] leading-tight whitespace-nowrap overflow-hidden text-ellipsis px-2">{name}</h3>
            <p className="text-primary text-[8px] sm:text-[9px] lg:text-[10px] tracking-[0.2em] sm:tracking-[0.3em] uppercase font-bold drop-shadow-md px-2">{role}</p>
          </motion.div>
        </motion.div>
      </motion.div>
    </AnimatedBlock>
